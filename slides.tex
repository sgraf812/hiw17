...
\begin{frame}[fragile]{Data-flow Framework for Strictness Analysis}
  \begin{overprint}
    ...
    \begin{columns}
      ...
      \end{column}
      \begin{column}{0.55\textwidth}
        \only<1>{$\{\alert{\dfnode{<root>}{0}}\}$}\only<2-5>{$\{\}$}\only<6>{$\{\alert<6>{\dfnode{f}{2}, \dfnode{<root>}{0}}\}$}\only<7,8>{$\{\alert<7,8>{\dfnode{f}{2}}\}$}\only<9->{$\{\}$}
      \end{column}
    \end{columns}
  \end{overprint}
\end{frame}
...
\begin{frame}[fragile]{Implementation}
  \begin{itemize}
    \item \hs{DataFlowFramework} assigns \hs{TransferFunction} and \hs{ChangeDetector} to nodes
  \end{itemize}
  \begin{center}
    \begin{minipage}{0.9\textwidth}
      \begin{haskell}
...
\begin{frame}[fragile]{Implementation}
  \begin{itemize}
    \item \hs{runFramework} solves data-flow problems 
    \item Specification as \hs{DataFlowFramework}
...
\begin{frame}[fragile]{Applied to Strictness Analysis}
  \begin{itemize}
    ...
    \item Instantiate as \hs{DataFlowFramework (ExprNode, Arity) StrType}
...
\begin{frame}[fragile]{Threading annotated expressions}
  \begin{itemize}
    ...
    \item Thread it through all nodes: \hs{DataFlowFramework (ExprNode, Arity) (StrType, CoreExpr)}
